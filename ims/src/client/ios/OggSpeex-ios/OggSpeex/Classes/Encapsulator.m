...
- (id)initWithFileName:(NSString *)filename :(id<ConsumerDelegate>) myConsumerDelegate {
    if (self = [super init]) {
        ...
        consumerDelegate = myConsumerDelegate;
    }
    return self;
}
...
- (void)prepareForEncapsulating {
    ...
    self.isCanceled = NO;
    encapsulationOperation = [[EncapsulatingOperation alloc] initWithParent:self :consumerDelegate];
    if (operationQueue) {
...
- (void)main {
    ...
    while ( ! self.mParent.isCanceled) {
        if ([[self.mParent getPCMDatas] count] > 0) {
            ...
            [consumerDelegate putData:speexData];
...
//初始化NSOperation
- (id)initWithParent:(Encapsulator *)parent :(id) delegate {
    if (self = [super init]) {
        self.mParent = parent;
