...
- (NSDictionary *)fontAttributes
{
	...
	CTFontSymbolicTraits symbolicTraits_ = [self symbolicTraits];
	
	if (symbolicTraits_)
	{
		[traitsDict setObject:[NSNumber numberWithUnsignedInt:symbolicTraits_] forKey:(id)kCTFontSymbolicTrait];
	}
...
- (void)setSymbolicTraits:(CTFontSymbolicTraits)symbolicTraits_
{
	if (symbolicTraits_ & kCTFontBoldTrait)
	{
		boldTrait = YES;
	}
	
	if (symbolicTraits_ & kCTFontItalicTrait)
	{
		italicTrait = YES;
	}
	
	if (symbolicTraits_ & kCTFontExpandedTrait)
	{
		expandedTrait = YES;
	}
	
	if (symbolicTraits_ & kCTFontCondensedTrait)
	{
		condensedTrait = YES;
	}
	
	if (symbolicTraits_ & kCTFontMonoSpaceTrait)
	{
		monospaceTrait = YES;
	}
	
	if (symbolicTraits_ & kCTFontVerticalTrait)
	{
		verticalTrait = YES;
	}
	
	if (symbolicTraits_ & kCTFontUIOptimizedTrait)
	{
		UIoptimizedTrait = YES;
	}
	
	// stylistic class is bundled in the traits
	stylisticClass = symbolicTraits_ & kCTFontClassMaskTrait;   
}
