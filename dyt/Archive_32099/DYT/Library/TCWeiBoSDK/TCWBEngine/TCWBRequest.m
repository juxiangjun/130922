		...
		if ([dataDictionary count] > 0) {
			for (id key in dataDictionary) {
				...
				if ([dataParam isKindOfClass:[UIImage class]]) {
					...
					[TCWBRequest appendUTF8Body:body dataString:[NSString stringWithFormat:@"Content-Type:\"image/jpeg\"\r\n\r\n"]];
...
- (id)parseJSONData:(NSData *)data error:(NSError **)error{
	...
	TCJSON *jsonParser = [[TCJSON alloc]init];
...
+ (TCWBRequest *)requestWithURL:(NSString *)url 
    ...
    TCWBRequest *request = [[[TCWBRequest alloc] init] autorelease];
